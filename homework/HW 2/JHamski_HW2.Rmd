...
3.  
```{r}
x.y <- seq(from = 0, to = 1, by = 0.01)
y.x <- seq(from = 1, to = 2, by = 0.01)

y <- (3/2) - x.y 
x <- y.x - (1/2)

joint.rv <- cbind(x.y, x, y.x, y) %>% as.data.frame()

ggplot(joint.rv) + geom_line(aes(x=x, y=x.y)) + geom_line(aes(x=y.x, y=y))
```
