...
The Sobol numbers are evenly distributed - they cover the distance from 0 to 1 evently. There are no major gaps or clusters. The random numbers (uniform distribution) for clusters where there are several random variables with close values and "dead space" where there are no random variables with the values. The difference between the two is their discrepency. The Sobol numbers are low discrepency but are "quasi-random" - the enforcement of even distribution means that the values are not i.i.d. and therefore not truly random. The uniform distribution random numbers have the potential to be high discrepancy. 

For D = 1:
```{r, cache=TRUE}
generate.x.list.Sobol <- function(n, D){
  sobol.seq <- (sobol(D*n) * 10) - 5 
  x <- matrix(sobol.seq, nrow=n, ncol=D)
  return(x)
}
```
