...
```{r}
ggplot(tidy.results.6.3) + geom_line(aes(x = Theta, y = Power, group = Sample.Size, color = Sample.Size))
```
...
get.lognorm <- function(n){
  x <- rlnorm(n, meanlog = runif(1,0,1), sdlog = runif(1, 0, 2))
  return(x)
}
```

getnorm() hides the lognormal parameters, I don't know what they are. However, with a sizable sample I can construct a confidence interval.
...
(CI <- c(x.bar - std.error, x.bar + std.error))
```
